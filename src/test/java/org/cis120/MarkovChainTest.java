...
public class MarkovChainTest {
    ...
    @Test
    //first element in bigram is null
    public void testAddBigramNullFirstElement() {
        MarkovChain mc = new MarkovChain();
        assertThrows(IllegalArgumentException.class, () -> mc.addBigram(null, ""));
    }
    @Test
    //second element in bigram is null
    public void testAddBigramNullSecondElement() {
        MarkovChain mc = new MarkovChain();
        assertThrows(IllegalArgumentException.class, () -> mc.addBigram("", null));
    }

    @Test
    //train an empty sentence
    public void trainTestEmpty() {
        ...
        assertTrue(mc.startWords.getEntrySet().isEmpty());
    }

    @Test
    // tests when reset input is a word not in sentence
    ...
    public void trainOneWord() {
        MarkovChain mc = new MarkovChain();
        String sentence = "Elda";
        mc.train(Arrays.stream(sentence.split(" ")).iterator());
        mc.reset();
        assertEquals(mc.next(), "Elda");
        assertFalse(mc.hasNext());
        assertThrows(IllegalArgumentException.class, () -> mc.addBigram("Elda", null));
    }

    @Test
    //> 2 sentences
    public void testBiggerWalk() {

        String[] expectedWords = {"CIS", "120", "crushes", "CIS"
                , "120", "beats", "CIS", "120", "rocks"};
        MarkovChain mc = new MarkovChain();

        String sentence1 = "CIS 120 rocks";
        String sentence2 = "CIS 120 beats CIS 160";
        String sentence3 = "CIS 120 crushes CIS 121";
        mc.train(Arrays.stream(sentence1.split(" ")).iterator());
        mc.train(Arrays.stream(sentence2.split(" ")).iterator());
