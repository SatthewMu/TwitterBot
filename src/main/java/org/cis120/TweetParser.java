...
public class TweetParser {
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (iter.hasNext()) {
            String words = extractColumn(iter.next(), tweetColumn);
            if (words != null) {
                tweets.add(words);
            }
        }
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        String[] cleanSentence = sentence.split(" ");
        List<String> parseSentence = new LinkedList<>();
        for (String s : cleanSentence) {
            String clean = cleanWord(s);
            if (clean != null) {
                parseSentence.add(clean);
            }
        }
        return parseSentence; // Complete this method.
    }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        List<String> tweets = tweetSplit(cleanTweet);
        List<List<String>> parseAndCleanTweets = new LinkedList<>();
        for (String s : tweets) {
            if (!parseAndCleanSentence(s).isEmpty()) {
                parseAndCleanTweets.add(parseAndCleanSentence(s));
            }
        }
        return parseAndCleanTweets; // Complete this method.
    }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        List<String> csvData = csvDataToTweets(br, tweetColumn);
        List<List<String>> tweets = new LinkedList<>();
        for (String s : csvData) {
            tweets.addAll(parseAndCleanTweet(s));
        }
        return tweets; // Complete this method
    }
