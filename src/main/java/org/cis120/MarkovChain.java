...
public class MarkovChain implements Iterator<String> {
    ...
    public void train(Iterator<String> sentence) {
        ...
        if (sentence == null) {
            ...
            currentS = sentence.next();
            if (!currentS.isEmpty()) {
                startWords.record(currentS);
            }
            while (sentence.hasNext()) {
                nextS = sentence.next();
                addBigram(currentS, nextS);
                currentS = nextS;
            }
            addBigram(currentS, END_TOKEN);
        }
        // Complete this method.
    }
    ...
    public String next() {
        if (hasNext()) {
            ...
            if (!chain.containsKey(next)) {
                ...
                current = next;
                ProbabilityDistribution<String> dist = chain.get(next);
                next = dist.pick(ng);
                return current;
            }
        } else {
